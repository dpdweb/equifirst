...
export default function MortgageCalculator() {
  ...
  const [state, setState] = useState({
    ...
    ActiveProduct: residentialStatus.RESIDENTS.id,
  ...
  const handleResidencyClick = (status: string) => {
    ...
    if (status === residentialStatus.NATIONAL.id) {
      ...
      updatedRate = 5.0
      const minDownpayment = state.Price >= thresholdPrice ? 40 : 35;
      const maxDownPayment = state.Price >= thresholdPrice ? 80 : 80;
      setDownPaymentPercentage(minDownpayment);
      setDownPaymentMixMax({ min: minDownpayment, max: maxDownPayment })
      newDownPayment = Math.round((state.Price * minDownpayment) / 100);
        ...
        <div className="md:pr-12">
                  ...
                  if (!isNaN(parsed)) {
                    ...
                    if (parsed >= thresholdPrice) {
                      ...
                      if (state.ActiveProduct === residentialStatus.NATIONAL.id) {
                        ...
                        setDownPaymentMixMax({ min: 35, max: 80 });
                        setDownPaymentPercentage(35);
                        downValue = Math.round((parsed * 25) / 100);
                      }
                    }
          ...
          <div className="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div className="flex items-center relative ">
              ...
              <div className="relative ml-2 mt-1 group">
                <button className="relative inline-block group focus:outline-none ">
                  <Info className="w-8 h-8 sm:w-4 sm:h-4 text-gray-500 cursor-pointer" />
                </button>
          ...
          <div className="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
            ...
            <div className="flex items-center space-x-2 relative">  
                <div className="font-semibold block items-center ">Would you like to finance your fees?</div>
            ...
            <button
              ...
              className={`w-12 h-6 flex items-center rounded-full p-1 ${state.ToggleFinancing ? "bg-ef-blue" : "bg-gray-300"
                }`}
            >
              <div
                className={`bg-white w-4 h-4 rounded-full shadow-md transform duration-300 ease-in-out ${state.ToggleFinancing ? "translate-x-6" : "translate-x-0"
                  }`}
              />
            </button>
          </div>
