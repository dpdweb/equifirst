...
export default function SubPageHeroBanner({ title, subtitle, image }: HeroBannerProps) {
  ...
  {/* Text Overlay */}
  <div className="relative z-10 px-6 pt-50 pb-50 md:pt-35 md:pb-40 md:px-16 text-white max-w-4xl">
    {title && (
      <p className="text-[16px] text-center md:text-left uppercase mb-2 tracking-wider">{title}</p>
    )}
    <h1 className="text-[36px] text-center md:text-left md:text-[64px] font-medium leading-15 whitespace-pre-line">
      {subtitle}
    </h1>
  </div>
</section>
